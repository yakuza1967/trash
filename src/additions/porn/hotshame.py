...
def hotshameGenreListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_CENTER | RT_VALIGN_CENTER, entry[0])
		]

def hotshameFilmListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_LEFT | RT_VALIGN_CENTER, entry[0])
		]

class hotshameGenreScreen(Screen):
	...
	def layoutFinished(self):
		self.keyLocked = True
		url = "http://www.hotshame.com/categories/"
	...
	def genreData(self, data):
		...
		if phCats:
			...
			self.genreliste.insert(0, ("Most Popular", "http://www.hotshame.com/most-popular/", None))
			self.genreliste.insert(0, ("Top Rated", "http://www.hotshame.com/top-rated/", None))
			self.genreliste.insert(0, ("Newest", "http://www.hotshame.com/", None))
			self.genreliste.insert(0, ("--- Search ---", "callSuchen", None))
			self.chooseMenuList.setList(map(hotshameGenreListEntry, self.genreliste))
	...
	def keyOK(self):
		...
		if streamGenreName == "--- Search ---":
			...
			self.session.open(hotshameFilmScreen, streamGenreLink)
	...
	def SuchenCallback(self, callback = None, entry = None):
		if callback is not None and len(callback):
			...
			streamGenreLink = 'http://www.hotshame.com/search/?q=%s' % self.suchString
			self.session.open(hotshameFilmScreen, streamGenreLink)
...
class hotshameFilmScreen(Screen):
	...
	def loadpage(self):
		...
		if search:
			url = 'http://www.hotshame.com/search/%s/%s' % (str(self.page), str(search.group(1)))
		else:
	...
	def loadData(self, data):
		...
		if phMovies:
			...
			self.chooseMenuList.setList(map(hotshameFilmListEntry, self.filmliste))
			self.chooseMenuList.moveToIndex(0)
			self.keyLocked = False
			self.showInfos()
	...
	def play(self,file):
		...
		self.session.open(SimplePlayer, [(xxxtitle, file)], showPlaylist=False, ltype='hotshame')
