...
def thenewpornGenreListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_CENTER | RT_VALIGN_CENTER, entry[0])
		]

def thenewpornFilmListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_LEFT | RT_VALIGN_CENTER, entry[0])
		]

class thenewpornGenreScreen(Screen):
	...
	def __init__(self, session):
		...
		self['title'] = Label("TheNewPorn.com")
	...
	def layoutFinished(self):
		self.keyLocked = True
		url = "http://www.thenewporn.com/categories/"
	...
	def keyOK(self):
		...
		if streamGenreName == "--- Search ---":
			...
			self.session.open(thenewpornFilmScreen, streamGenreLink)
	...
	def SuchenCallback(self, callback = None, entry = None):
		if callback is not None and len(callback):
			...
			streamGenreLink = 'http://www.thenewporn.com/search/?q=%s' % self.suchString
			self.session.open(thenewpornFilmScreen, streamGenreLink)
...
class thenewpornFilmScreen(Screen):
	...
	def loadpage(self):
		...
		if search:
			url = 'http://www.thenewporn.com/search/%s/%s' % (str(self.page), str(search.group(1)))
		else:
