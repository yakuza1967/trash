...
def sunpornoGenreListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_CENTER | RT_VALIGN_CENTER, entry[0])
		]

def sunpornoFilmListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_LEFT | RT_VALIGN_CENTER, entry[0])
		]

class sunpornoGenreScreen(Screen):
	...
	def genreData(self, data):
		phCats = re.findall('class="categories">.*?href="(.*?)1.html".*?img\ssrc="(.*?)".*?alt="(.*?)"', data, re.S)
		if phCats:
			for (phUrl, phImage, phTitle) in phCats:
				self.genreliste.append((phTitle, phUrl, phImage))
			self.genreliste.sort()
			self.genreliste.insert(0, ("High Definition", "http://www.sunporno.com/high-definition/page", None))
			self.genreliste.insert(0, ("Longest", "http://www.sunporno.com/long-movies/page", None))
			self.genreliste.insert(0, ("Most Favorited", "http://www.sunporno.com/mostfavorites/page", None))
			self.genreliste.insert(0, ("Top Rated", "http://www.sunporno.com/top-rated/page", None))
			self.genreliste.insert(0, ("Newest", "http://www.sunporno.com/page", None))
			self.genreliste.insert(0, ("--- Search ---", "callSuchen", None))
			self.chooseMenuList.setList(map(sunpornoGenreListEntry, self.genreliste))
	...
	def keyOK(self):
		...
		if streamGenreName == "--- Search ---":
			...
			self.session.open(sunpornoFilmScreen, streamGenreLink)
	...
	def SuchenCallback(self, callback = None, entry = None):
		if callback is not None and len(callback):
			...
			streamGenreLink = 'http://www.sunporno.com/search/%s/page' % (self.suchString)
			self.session.open(sunpornoFilmScreen, streamGenreLink)
...
class sunpornoFilmScreen(Screen):
	...
	def loadData(self, data):
		...
		parse = re.search('title="By\sduration"(.*?)</html>', data, re.S)
		phMovies = re.findall('divplusarea.*?href="(.*?)".*?title="(.*?)".*?img\ssrc="(.*?)".*?class="fl1"><b>(.*?)</b>', parse.group(1), re.S)
		if phMovies:
			for (phUrl, phTitle, phImage, phRuntime) in phMovies:
				self.filmliste.append((decodeHtml(phTitle), phUrl, phImage, phRuntime))
			self.chooseMenuList.setList(map(sunpornoFilmListEntry, self.filmliste))
			self.chooseMenuList.moveToIndex(0)
			self.keyLocked = False
			self.showInfos()
