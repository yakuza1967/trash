...
def pornrabbitGenreListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_CENTER | RT_VALIGN_CENTER, entry[0])
		]

def pornrabbitFilmListEntry(entry):
	return [entry,
		(eListboxPythonMultiContent.TYPE_TEXT, 20, 0, 900, 25, 0, RT_HALIGN_LEFT | RT_VALIGN_CENTER, entry[0])
		]

class pornrabbitGenreScreen(Screen):
	...
	def genreData(self, data):
		phCats = re.findall('class="cat">.*?<a href="(.*?)".*?<h2>(.*?)<small>.*?<img src="(.*?)"', data, re.S)
		if phCats:
	...
	def keyOK(self):
		...
		if streamGenreName == "--- Search ---":
			...
			self.session.open(pornrabbitFilmScreen, streamGenreLink)
...
class pornrabbitFilmScreen(Screen):
	...
	def loadData(self, data):
		x = 0
		phMovies = re.findall('class="video">.*?<a href="(.*?)" title="(.*?)".*?<img.*?src="(.*?)".*?views: <b>(.*?)</b>.*?runtime: <b>(.*?)</b>', data, re.S)
		if phMovies:
			for (phUrl, phTitle, phImage, phViews, phRuntime) in phMovies:
				if not re.match('/video/3',phUrl):
					x = x + 1
	...
	def keyOK(self):
		...
		if not phLink == None:
			url = 'http://www.pornrabbit.com%s' % phLink
			self.keyLocked = True
	...
	def play(self,file):
		...
		self.session.open(SimplePlayer, [(xxxtitle, file)], showPlaylist=False, ltype='pornrabbit')
