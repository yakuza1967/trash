	...
	def get_series_more_pages(self, data, reitername):
		...
		if folgen:
			for (url,title, sperre) in folgen:
				print title
				url = "http://www.n-tvnow.de" + url.replace('amp;','')
			...
			self.chooseMenuList.setList(map(NTVnowFilmListEntry, self.filmliste))
			self.keyLocked = False
	...
	def get_stream(self, data):
		...
		if rtmpe_data:
			...
			if config.mediaportal.useRtmpDump.value:
				final = "%s' --swfVfy=1 --playpath=mp4:%s --app=ntvnow/_definst_ --pageUrl=http://www.n-tvnow.de/ --tcUrl=rtmpe://fms-fra34.rtl.de/ntvnow/ --swfUrl=http://www.n-tvnow.de/includes/vodplayer.swf'" % (host, playpath)
				print final
